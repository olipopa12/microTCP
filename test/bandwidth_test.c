...
int server_microtcp(uint16_t listen_port, const char *file)
{
 ...
 // sock.state = LISTEN;
  ...
  if (accepted < 0)
  {
    perror("Error in microtcp accept.");
	free(buffer);
	fclose(fp);
    return -1;
  }
  ...
  */
  ...
  {
	 // printf("received:%d\n", received);
	  //if (received == 0) //no received bytes because of 3 dup ack or time out
		//  continue;
	  
	  written = fwrite(buffer, sizeof(uint8_t), received, fp); //don't write the header
	  total_bytes += received;
	  if (written * sizeof(uint8_t) != received)
	  {
		  printf("Failed to write to the file the"
...
//	  printf("data sent,read items*sizeof():%d,%d\n", data_sent, read_items * sizeof(uint8_t) + sizeof(microtcp_header_t));
	  if (data_sent != read_items * sizeof(uint8_t))
	  {
		  printf("Failed to send the"
			  " amount of data read from the file.\n");
